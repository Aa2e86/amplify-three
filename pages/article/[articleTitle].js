...
export async function getServerSideProps(context) {
  ...
  const { articleTitle } = params; // replace with your own parameter name

  const SSRContext = withSSRContext();
  const [fetchedArticle] = await SSRContext.DataStore.query(Article, c => c.urltitle.eq(articleTitle));
  
  return {
    props: {
      article: JSON.parse(JSON.stringify(fetchedArticle)),
    },
  };
}
...
export default function ArticlePage() {
  ...
  const name = typeof articleTitle === 'string' ? articleTitle : '';
